...
func Verify(email *[]byte, opts ...DNSOpt) (verifyOutput, error) {
	...
	// pubKey, verifyOutputOnError, err := NewPubKeyResp("v=DKIM1;k=rsa;p=;n=2048,1452627113,1468351913")
	// pubKey, verifyOutputOnError, err := NewPubKeyResp("v=DKIM1; k=rsa; p=MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDPsFIOSteMStsN615gUWK2RpNJ/B/ekmm4jVlu2fNzXADFkjF8mCMgh0uYe8w46FVqxUS97habZq6P5jmCj/WvtPGZAX49jmdaB38hzZ5cUmwYZkdue6dM17sWocPZO8e7HVdq7bQwfGuUjVuMKfeTB3iNeo6/hFhb9TmUgnwjpQIDAQAB")
	pubKey, verifyOutputOnError, err := NewPubKeyResp("v=DKIM1; k=rsa; p=")
	// fmt.Print("[")
	// for i := 0; i < len(pubKey.PubKey.N.Bytes()); i++ {
	// 	fmt.Print(pubKey.PubKey.N.Bytes()[i])
	// 	fmt.Print(", ")
	// }
	// fmt.Println("]")
	// fmt.Println(pubKey.PubKey.E)
	if err != nil {
	...
	fmt.Println("toSign=------------" + string(toSign) + "-------------")
	fmt.Println("sign=-----------" + dkimHeader.SignatureData + "--------------")
...
func getBodyHash(body *[]byte, algo string, bodyLength uint) (string, error) {
	...
	fmt.Println("bbbbbb-------" + string(toH) + "-------bbbbbb")
